...
define('zapp_user_main', ['dart_sdk', 'flutter_sdk'], (function load__zapp_user_main(dart_sdk, flutter_sdk) {
  ...
  dart.defineLazy(CT, {
    ...
    get C4() {
      return C[4] = dart.constList([C[5] || CT.C5, C[6] || CT.C6, C[7] || CT.C7, C[8] || CT.C8], types.LaunchMode);
    },
    get C9() {
  ...
  main.MyApp = class MyApp extends framework.StatelessWidget {
    build(context) {
      return new app.MaterialApp.new({title: "Flutter Demo", theme: theme_data.ThemeData.new({primarySwatch: colors.Colors.green}), home: new main.MyHomePage.new()});
    }
  ...
  var _likeCount = dart.privateName(main, "_likeCount");
  var _LikeCountPlus = dart.privateName(main, "_LikeCountPlus");
  var _showRoute = dart.privateName(main, "_showRoute");
  var _shareContent = dart.privateName(main, "_shareContent");
  var _call = dart.privateName(main, "_call");
  main._MyHomePageState = class _MyHomePageState extends framework.State$(main.MyHomePage) {
    [_LikeCountPlus]() {
      this.setState(dart.fn(() => {
        if (this.k === 0) this[_likeCount] = this[_likeCount] + 1;
        this.k = this.k + 1;
      }, T.VoidTovoid()));
    }
    [_showRoute]() {
      return async.async(dart.void, (function* _showRoute() {
        let url = "https://yandex.ru/maps/35/krasnodar/search/%D0%BA%D1%83%D0%B1%D0%B3%D0%B0%D1%83/?ll=38.932804%2C45.045469&sctx=ZAAAAAgBEAAaKAoSCYwubw7XfENAEZv%2B7EeKhEZAEhIJ%2BaI9XkiH2D8R6xwDste7xz8iBgABAgMEBSgKOABAI0gBagJydZ0BzczMPaABAKgBAL0BH1DdLcIBD5iAiL0ElM%2Fk8AXijam5CIICDNC60YPQsdCz0LDRg4oCAJICAJoCDGRlc2t0b3AtbWFwcw%3D%3D&sll=38.932804%2C45.045469&source=serp_navig&sspn=0.031924%2C0.013316&z=15.5";
        if (yield legacy_api.canLaunch(url)) {
          yield legacy_api.launch(url);
        } else {
          scaffold.ScaffoldMessenger.of(this.context).showSnackBar(new snack_bar.SnackBar.new({content: new text.Text.new("Не удалось открыть карту.")}));
        }
      }).bind(this));
    }
    [_shareContent]() {
      scaffold.ScaffoldMessenger.of(this.context).showSnackBar(new snack_bar.SnackBar.new({content: new text.Text.new("Поделиться контентом...")}));
    }
    [_call]() {
      scaffold.ScaffoldMessenger.of(this.context).showSnackBar(new snack_bar.SnackBar.new({content: new text.Text.new("8-800-222-22-22")}));
    }
    build(context) {
      return new scaffold.Scaffold.new({appBar: new app_bar.AppBar.new({title: new text.Text.new("Кампус Кубанского ГАУ"), backgroundColor: colors.Colors.green}), body: new single_child_scroll_view.SingleChildScrollView.new({padding: new edge_insets.EdgeInsets.all(16), child: new basic.Column.new({crossAxisAlignment: flex.CrossAxisAlignment.start, children: T.JSArrayOfWidget().of([new image.Image.network("https://sun9-40.userapi.com/s/v1/ig2/m8wRvQ70t4diy--ngylsjzqPH0Q0zKd_zXn2MDsAG7jW6_HqWtmFZS9vQ-jA2h6Ybe842VvvBt7WkXUWPvdXymqG.jpg?quality=95&as=32x23,48x34,72x51,108x77,160x114,240x171,360x257,480x343,540x385,640x457,720x514,1080x771,1280x913,1435x1024&from=bu&u=vkdmz6YeWjWtphWD_EJyCAmvnI8spNDZd83OvRNVUpU&cs=604x431", {width: 1 / 0, height: 300, fit: box_fit.BoxFit.cover}), new basic.Row.new({children: T.JSArrayOfWidget().of([new text.Text.new("Общежитие №20", {style: new text_style.TextStyle.new({fontSize: 16, fontWeight: ui.FontWeight.bold})}), new basic.SizedBox.new({width: 220}), new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.favorite, {color: colors.Colors.red}), onPressed: dart.bind(this, _LikeCountPlus)}), new basic.SizedBox.new({width: 8}), new text.Text.new(dart.str(this[_likeCount]), {style: new text_style.TextStyle.new({fontSize: 18, color: colors.Colors.black})})])}), new text.Text.new("Краснодар, ул. Калинина 13", {style: new text_style.TextStyle.new({fontSize: 13})}), new basic.SizedBox.new({height: 16}), new basic.Row.new({mainAxisAlignment: flex.MainAxisAlignment.center, children: T.JSArrayOfWidget().of([new basic.Column.new({children: T.JSArrayOfWidget().of([new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.phone, {color: colors.Colors.green}), onPressed: dart.bind(this, _call)}), new text.Text.new("Позвонить")])}), new basic.SizedBox.new({width: 70}), new basic.Column.new({children: T.JSArrayOfWidget().of([new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.navigation, {color: colors.Colors.green}), onPressed: dart.bind(this, _showRoute)}), new text.Text.new("Маршрут")])}), new basic.SizedBox.new({width: 70}), new basic.Column.new({children: T.JSArrayOfWidget().of([new icon_button.IconButton.new({icon: new icon.Icon.new(icons.Icons.share, {color: colors.Colors.green}), onPressed: dart.bind(this, _shareContent)}), new text.Text.new("Поделиться")])})])}), new basic.SizedBox.new({height: 16}), new text.Text.new("Студенческий городок или так называемый кампус Кубанского ГАУ состоит " + "из двадцати общежитий, в которых проживает более 8000 студентов, что состав-" + "ляет 96% от всех нуждающихся. Студенты первого курса обеспечены местами в " + "общежитии полностью. В соответствии с Положением о студенческих общежитиях " + "университета, при поселении между администрацией и студентами заключается " + "договор найма жилого помещения. Воспитательная работа в общежитиях " + "направлена на улучшение быта, соблюдение правил внутреннего распорядка, отсутствия " + "асоциальных явлений в молодежной среде. Условия проживания в общежитиях " + "университетского кампуса полностью отвечают санитарным нормам и " + "требованиям: наличие оборудованных кухонь, душевых комнат, прачечных, читальных " + "залов, комнат самоподготовки, помещений для заседаний студенческих советов и " + "наглядной агитации. С целью улучшения условий быта студентов активно работает " + "система студенческого самоуправления - студенческие советы организуют всю " + "работу по самообслуживанию.", {style: new text_style.TextStyle.new({fontSize: 16})})])})})});
    }
    static ['_#new#tearOff']() {
      return new main._MyHomePageState.new();
    }
  ...
  dart.setMethodSignature(main._MyHomePageState, () => ({
    ...
    [_LikeCountPlus]: dart.fnType(dart.void, []),
    [_showRoute]: dart.fnType(dart.void, []),
    [_shareContent]: dart.fnType(dart.void, []),
    [_call]: dart.fnType(dart.void, []),
    build: dart.fnType(framework.Widget, [framework.BuildContext])
  }));
  ...
  dart.setFieldSignature(main._MyHomePageState, () => ({
    ...
    [_likeCount]: dart.fieldType(core.int),
    k: dart.fieldType(core.int)
  }));
  ...
  url_launcher_uri.launchUrl = function launchUrl(url, opts) {
    let mode = opts && 'mode' in opts ? opts.mode : C[5] || CT.C5;
    let webViewConfiguration = opts && 'webViewConfiguration' in opts ? opts.webViewConfiguration : C[9] || CT.C9;
    let webOnlyWindowName = opts && 'webOnlyWindowName' in opts ? opts.webOnlyWindowName : null;
